...
• Perintah: <code>{cobadah}info</code> [user_id/username/reply to users]
• Penjelasan: Untuk mendapatkan info pengguna telegram dengan deskripsi lengkap.

• Perintah: <code>{cobadah}cinfo</code> [chat_id/username/reply to chat]
...
@ubot.on_message(filters.me & anjay(["whois", "info"]))
...
@ubot.on_message(filters.me & anjay(["cwhois", "cinfo"]))
async def _(client, message):
    Tm = await eor(message, "</b>Processing . . .</b>")
    try:
